...
def persam(args, obj_name, images_path, masks_path, output_path):
    ...
    for test_idx in tqdm(range(len(os.listdir(test_images_path)))):
        ...
        best_iou = 0
        best_output = None
        ious_0 = [iou (masks0 [midx0], test_mask) for midx0 in range (masks0.shape [0])]
        pred_iou0 = np.argmax (ious_0)
        for midx0 in range (masks0.shape [0]):
            ...
            ious_1 = [iou (masks1 [midx1], test_mask) for midx1 in range (masks1.shape [0])]
            pred_iou1 = np.argmax (ious_1)
            for midx1 in range (masks1.shape [0]):
                ...
                ious_2 = [iou (masks2 [midx2], test_mask) for midx2 in range (masks2.shape [0])]
                pred_iou2 = np.argmax (ious_2)
                for midx2 in range (masks2.shape [0]):
                    ...
                    if pred_iou0 == midx0 and pred_iou1 == midx1 and pred_iou2 == midx2:
                        pred_output_path = os.path.join(output_path, f'best_pred_{test_idx}.png')
                        cv2.imwrite(pred_output_path, concatenated_img)
